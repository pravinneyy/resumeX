...
import { BarChart, Trophy, TrendingUp, Users, ArrowUpRight } from "lucide-react"
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // 2. Fetch Applications for ALL jobs to aggregate data
        // (In a real production app, you'd make a dedicated /api/analytics endpoint)
        await Promise.all(jobs.map(async (job: any) => {
            const res = await fetch(`http://127.0.0.1:8000/api/jobs/${job.id}/applications`)
            if (res.ok) {
                const apps = await res.json()
                // Attach job title to app for context
        ...
        setStats({
            ...
            avgScore: Number(avg),
            topCandidates: top,
            recentActivity: allApps.slice(0, 5) // Just taking first 5 for now
        })
      } catch (e) {
        console.error(e)
      } finally {
